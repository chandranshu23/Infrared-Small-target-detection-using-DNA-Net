...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "Misc" in img_id:
            img_path   = self.images+'/'+img_id+".png"
        else:
            img_path   = self.images+'/'+img_id+".jpg"   # img_id的数值正好补了self._image_path在上面定义的2个空

        if "Misc" in img_id:
            label_path   = self.masks+'/'+img_id+"_pixels0.png"
        elif "output_image" in img_id:
            label_path   = self.masks+'/'+img_id+".png"
        else:
            label_path   = self.masks+'/'+img_id+"_masks.png"
...
def total_visulization_generation(dataset_dir, mode, test_txt, suffix, target_image_path, target_dir):
    ...
    for i in range(len(ids)):
        ...
        if "Misc" in ids[1]:
            source_image = source_image_path+'/'+ids[1]+".png"
        else:
            source_image = source_image_path+'/'+ids[1]+".jpg"
